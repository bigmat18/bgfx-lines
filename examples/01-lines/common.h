...
void generatePointsInSphere(std::vector<lines::LinesVertex> &points, float radius, float n) {
    ...
    for (uint i = 0; i < n; i++) {
        ...
        points.push_back(lines::LinesVertex(x0, y0, z0, lines::LinesVertex::COLOR(color(gen), color(gen), color(gen), 1.0)));
        points.push_back(lines::LinesVertex(x1, y1, z1, lines::LinesVertex::COLOR(color(gen), color(gen), color(gen), 1.0)));
    }
}
