...
IMAGE2D_RO(u_Texture, rgba32f, 0);
...
void main() {
    float baseIndex = gl_InstanceID * 3;

    vec4 p0 =    imageLoad(u_Texture, ivec2(baseIndex, 0));
    vec4 p1 =    imageLoad(u_Texture, ivec2(baseIndex + 1, 0));
    vec4 color = imageLoad(u_Texture, ivec2(baseIndex + 2, 0));
